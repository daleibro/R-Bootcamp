...
```{r Prep_Data}
drop_cols_data <- c("StatZoneSort", "StatZoneCd", "QuarSort", "QuarCd", "KreisSort", "KreisCd", "ProjStatusSort", "EigentumsartSort")
...
pricesreduced <- prices %>%
  ...
  select(- filter(.prices, Typ = "Zahl")
  
#head(pricesreduced, n = 10)
```

```{r Combine}
#datacomb <- left_join(data, prices, by = "QuarLang", "Jahr")
#head(datacomb, n = 10)
```
